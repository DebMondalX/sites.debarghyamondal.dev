#!/usr/bin/env bash
# add-skill.sh — install the sites.debarghyamondal.dev publisher skill into your AI agents.
#
# One-liner (installs into all detected agents):
#   curl -fsSL https://sites.debarghyamondal.dev/add-skill.sh | bash
#
# Pick specific agents:
#   curl -fsSL https://sites.debarghyamondal.dev/add-skill.sh | bash -s -- --only claude,pi
#
# Remove it again:
#   curl -fsSL https://sites.debarghyamondal.dev/add-skill.sh | bash -s -- --uninstall
set -euo pipefail

SKILL="publish-site-artifact"
BASE_URL="${ADD_SKILL_BASE_URL:-https://sites.debarghyamondal.dev}"
