...
description: Publish a static site artifact — a self-contained report, guide, or plan page — to the static-site dumpyard. Use when asked to publish, upload, or add a static page artifact.
---

# Publish to sites.debarghyamondal.dev

Static-site dumpyard by Debarghya Mondal. Repo `DebMondalX/sites.debarghyamondal.dev`, branch `main`, served from root at https://sites.debarghyamondal.dev.
...
2. Write self-contained `index.html` (inline CSS/JS, no build, no frameworks; keep any assets inside your folder) to:
   ...
   - plan → `plans/<slug>/index.html` (serves at `/plans/<slug>/`)
3. Never touch `CNAME`, `.nojekyll`, `robots.txt`, `llms.txt`, `add-skill.sh`, `skills/`, `research/index.html`, `learning/index.html`, `plans/index.html` (listings regenerate automatically via a GitHub Action). Never commit tokens/secrets.
...
gh api "repos/DebMondalX/sites.debarghyamondal.dev/contents/research/<slug>/index.html" \
...
Call `create_or_update_file` once per file with owner `DebMondalX`,
repo `sites.debarghyamondal.dev`, branch `main`, path `research/<slug>/index.html`
(or `learning/...`, `plans/...`), your HTML as `content` (raw text, not base64),
...
curl -X PUT -H "Authorization: Bearer $GITHUB_TOKEN" \
  ...
  https://api.github.com/repos/DebMondalX/sites.debarghyamondal.dev/contents/research/<slug>/index.html \
